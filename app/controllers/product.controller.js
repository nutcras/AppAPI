...
exports.create = async (req, res) => {
  ...
  const { pname, image, description } = req.body
  //ตรวจสอบความถูกต้อง request
  if (validate_req(req, res, [pname, image,description])) return
  //คำสั่ง SQL
...
exports.update = async (req, res) => {
  ...
  //คำสั่ง SQL
  let sql = `UPDATE product SET product_name = ?,product_image  = ?,product_description = ? WHERE product_id = ?`
  //ข้อมูลที่จะแก้ไขโดยเรียงตามลำดับ เครื่องหมาย ?
  let data = [pname, image, description, id]
