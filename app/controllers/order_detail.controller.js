...
exports.create = async (req, res) => {
  ...
  const { qty, dep,  } = req.body
  //ตรวจสอบความถูกต้อง request
  if (validate_req(req, res, [qty, dep])) return
  //คำสั่ง SQL
...
exports.findOne = async (req, res) => {
  ...
  //คำสั่ง SQL
  let sql = `SELECT * FROM order_detail WHERE orderD_id = ${id}`
  //ดึงข้อมูล โดยส่งคำสั่ง SQL เข้าไป
  await mysql.get(sql, (err, data) => {
    if (err)
      res.status(err.status).send({
...
exports.update = async (req, res) => {
  ...
  //คำสั่ง SQL
  let sql = `UPDATE order_detail SET orderD_qty = ?,orderD_dep  = ? = ? WHERE orderD_id = ?`
...
exports.deleteOne = async (req, res) => {
  ...
  if (validate_req(req, res, [id])) return
  //คำสั่ง SQL
  let sql = `DELETE FROM order_detail WHERE orderD_id = ?`
  //ข้อมูลที่จะแก้ไขโดยเรียงตามลำดับ เครื่องหมาย ?
  let data = [id]
